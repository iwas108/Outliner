...
### 9. NLP-Powered Keyword Chaining & PDF Clickable Footer Link
- **compromise NLP Library**: Installed the `compromise` npm package (client-side NLP, zero external calls) to enable part-of-speech (POS) tagging in the keyword analysis engine.
- **Strict Chaining Scope**: Refactored `checkKeywordChaining()` and `getKeywordColors()` in `analyzer.ts` to compare each Q/A node only against its **direct parent node** and **same-depth siblings under the same parent**. Previously, all Q/A nodes were compared globally, causing irrelevant keywords to flood highlights across unrelated sections.
- **POS-Consistent Highlighting**: Added `getWordPOSTags()` which uses compromise's `.compute('penn')` to assign Penn Treebank part-of-speech tags per word. In `getKeywordColors()`, a shared keyword between two lines is only highlighted if both lines use it in the **same POS category** (e.g., both as Noun). Words used as a Noun in one line and a Verb in another are suppressed — preventing misleading highlights.
- **PDF Clickable Footer Link**: Replaced the CSS `@page @bottom-left` margin box (which is plain-text only per the CSS spec) with a `position: fixed` `.page-footer` div injected into the print body. This footer contains a real `<a href="...">Made with Outlined ↗</a>` hyperlink that repeats on every printed page. The revision label was moved from `@bottom-center` into this fixed footer for consistency.

### 24. First-Question Chaining Exclusion, PDF Footer Page Alignment & Optimized Export Rendering (June 8, 2026)
- **First Question Chaining Exclusion & Subsequent Question Chaining**: Refactored `getChainingPartners` in `analyzer.ts` to identify if a Q/A node is the very first child of a Section (depth 0) or Topic (depth 1) parent. If it is the first Q/A node under the topic, it returns no partners, preventing false-positive keyword chaining violations in the diagnostics panel. For subsequent questions (depth 2) under a topic, configured them to keyword-chain directly against the answers and multi-level/multi-answer sub-answers of the immediately preceding question block.
- **Node-Scoped Chaining Highlights**: Scoped keyword color mappings returned by `getKeywordColors` in `analyzer.ts` to specific node IDs. This ensures that keywords are only highlighted on lines where they chain locally within their own topic, resolving global inter-topic highlight leaks.
- **Unfocus-Only Analysis Trigger**: Put `getKeywordColors` under a state and a `useEffect` hook in `OutlineIDE.tsx` that only triggers when `focusedIndex === null` (i.e. on field unfocus or clicking empty page). This completely prevents expensive compromise.js/nlp calculations from executing during typing, resolving editor keying lag.
- **PDF Page Number Footer Alignment**: In `pdfPrinter.ts`, added a dedicated `<span class="page-footer-page"></span>` element inside the fixed `.page-footer` container. This joins the page counter with "Made with" and "Revision" elements on a single, perfectly spaced line using CSS flexbox layout.
- **Optimized Export View & Switch Loader**: Optimized `ExportView.tsx` by using `useMemo` to compute keyword colors once per project update instead of re-calculating them inside the loop for every line. Added a simple "Please wait" spinner inside `OutlineIDE.tsx` to handle the transition smoothly and let the user wait when switching to the Export tab.

### 25. Word Document Export (.docx) (June 8, 2026)
- **Word Document Generator ([docxPrinter.ts](file:///home/madman/github/Outliner/src/utils/docxPrinter.ts))**: Built a TypeScript document printer utilizing the `docx` library. Translates the outline project data, margins, orientation, and page size into OpenXML properties.
- **Header Table Layout**: Styled the project metadata (goal, audience, objective, research questions) into a professional, shaded 2-column Word table with left accent borders matching the PDF layout design.
- **Outline Formatting & Spacing**: Mapped line spacing configurations to `LineRuleType.AUTO` paragraph spacing and computed indented left margins dynamically based on node levels.
- **Keyword Highlight Mapping**: Parsed the dynamic HSL keyword highlight color tags into standard hex colors and mapped them onto individual `TextRun` shading backgrounds inside Word documents.
- **UI Integration**: Added an "Export to Word (.docx)" action button inside the "File Export Actions" sidebar card of [ExportView.tsx](file:///home/madman/github/Outliner/src/components/IDE/ExportView.tsx).

### 26. Local MathJax Integration (June 10, 2026)
- **Zero-CDN MathJax v4 Package**: Installed `mathjax` locally as an npm dependency and configured a `postinstall` script in `package.json` to copy built assets into the public asset folder (`public/mathjax/`). This enables 100% offline-first operations and completely eliminates remote CDN calls.
- **Granular Math Renderer Component ([MathJaxRenderer.tsx](file:///home/madman/github/Outliner/src/components/IDE/MathJaxRenderer.tsx))**: Created an optimized MathJax wrapper component using React refs and element-scoped typesetting (`window.MathJax.typesetPromise([el])`). Typesetting triggers only when the text content changes, preventing UI lag.
- **Focus-Controlled Editor Previews ([OutlineLineItem.tsx](file:///home/madman/github/Outliner/src/components/IDE/OutlineLineItem.tsx))**: Linked `MathJaxRenderer` to Question and Answer fields when unfocused. While active, fields render as standard textareas to allow seamless LaTeX code editing without any rendering latency.
- **Export Preview & Mockup Typesetting ([ExportView.tsx](file:///home/madman/github/Outliner/src/components/IDE/ExportView.tsx))**: Embedded `MathJaxRenderer` into Question and Answer fields on the PDF visual mockup sheet, ensuring mathematical formulas display correctly on the Export page.
- **Typeset-Aware Vector Printing ([pdfPrinter.ts](file:///home/madman/github/Outliner/src/utils/pdfPrinter.ts))**: Loaded local MathJax configurations in the hidden print iframe. Delayed the print dialog trigger until `MathJax.startup.promise` resolves to ensure formulas are rendered in the generated PDF. Integrated a post-print `window.postMessage` listener in the parent frame to execute cleanup and restore application titles safely.

### 27. Generalized Sequential Keyword-Chaining Logic (June 10, 2026)
- **Sequential Q→A Chaining at All Depths**: Refactored `getChainingPartners` in `analyzer.ts` to generalize the sequential chaining model from depth-2-only to all Q/A depths. Previously, only the first level of questions (depth 2) chained with the answer of the previous question. Now every question node at any depth (2, 4, 6, ...) chains with the answer(s) of the immediately preceding sibling question under the same parent, ensuring consistent cohesion and coherence across nested sub-topics.
- **Answer → Parent Question Chaining**: Answer nodes now chain exclusively with their parent question node (depth - 1), providing direct Q → A coherence rather than the old sibling-based matching.
- **Improved Violation Messages**: Updated diagnostic violation messages to be context-specific: questions report missing chaining with the previous answer, and answers report missing chaining with their parent question.

### 28. Multi-Answer Sibling Keyword-Chaining Bug Fix (June 10, 2026)
- **Bug**: When a question had multiple answers, the 2nd, 3rd, etc. answer nodes all chained exclusively against the parent question. This meant if answer 2 diverged slightly from the question's exact wording but was cohesive with answer 1, it would still raise a false-positive chaining violation.
- **Fix**: Refactored the answer-node branch in `getChainingPartners` in [analyzer.ts](file:///home/madman/github/Outliner/src/utils/analyzer.ts) to detect whether a preceding sibling answer exists under the same parent. For the **first** answer, behavior is unchanged — it chains with the parent question only. For **2nd+ answers**, the function now returns **both** the immediately preceding sibling answer **and** the parent question as chaining partners. A violation is only raised if the answer shares no keywords with *either* partner, satisfying the "previous sibling answer OR parent question" rule.
- **Keyword Color Highlights**: The same `getChainingPartners` function drives `getKeywordColors`, so shared-keyword highlights now correctly appear between consecutive sibling answers, improving visual coherence in the editor overlay.

### 29. PDF Export Footer Margin Synchronization Fix (June 10, 2026)
