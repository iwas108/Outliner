...
2. **Keyword Chaining Constraints**: Enforces sequential cohesion within topics and nested sub-topics at every depth level. **Question nodes** must share at least one content keyword (excluding standard English stop words) with the *answer(s) of the immediately preceding sibling question* under the same parent. **Answer nodes** must share at least one keyword with their *parent question*. The first question under any parent has no chaining requirement (it introduces a new thread). **Section (depth 0) and Topic (depth 1) nodes are excluded** from all chaining validation.
